...
def create_order(order_data: OrderCreate, db: Session = Depends(get_db)):
    ...
    if not customer:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Customer with id {order_data.customer_id} not found")
    ...
    for item in order_data.items:
        ...
        if product.quantity < item.quantity:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Insufficient stock for '{product.name}'. Available: {product.quantity}, Requested: {item.quantity}")
    ...
    for (product, quantity), item in zip(products_to_update, order_data.items):
        db.add(OrderItem(order_id=db_order.id, product_id=item.product_id, quantity=quantity, unit_price=product.price))
